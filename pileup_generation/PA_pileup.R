setwd("~/Documents/s4U_final_mapped/new_p_aeruginosa_CCA_bam/") #working directory containing reference.fa, reference.fai, and subfolders for each condition containing replicate BAMs + indexes
library(data.table)
library(plyr)
library(tidyverse)
library(Rsamtools)
...
tRNA_seqs <-
  open(
    FaFile(
      "Paeruginosa_transcriptome_uniquetRNA_tRNA-sec_edit.fa",
      "Paeruginosa_transcriptome_uniquetRNA_tRNA-sec_edit.fa.fai"
    )
  )
...
for (experiment in list_of_experiments)
{
  ...
  raw_output_name <-
    paste(exp_name, "collated_freqs_CCA_NEW.csv", sep = "_")
  summary_output_name <-
    paste(exp_name, "summary_freqs_CCA_NEW.csv", sep = "_")
  filtered_output_name <-
    paste(exp_name, "filtered_freqs_CCA_NEW.csv", sep = "_")
  ...
  summary_pileup_output <- summary_pileup_output %>% rowwise() %>%
    mutate(tRNA_name = sub("_.*", "", tRNA_index))
  summary_pileup_output <- summary_pileup_output %>% rowwise() %>%
    mutate(tRNA_position = as.numeric(sub(".*_", "", tRNA_index)))
  colnames(raw_pileup_output) <-
    paste(colnames(raw_pileup_output),
          as.character(experiment),
          sep = "_")
  filtered_pileup_output <- summary_pileup_output %>% rowwise() %>%
    mutate(misincorporation_freq = case_when(avg_depth < 100 ~ NA,
                                             TRUE ~ misincorporation_freq))
